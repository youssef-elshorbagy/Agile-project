...
const manageEnrollment = async (req, res) => {
    try {
        // 1. Get the new ID from the frontend
        ...
        // 2. Run the Update using the UNIQUE EnrollmentID
        if (status === 'enrolled') {
             await sql.query`
                UPDATE Enrollments 
                SET status = 'enrolled' 
            ...
            // Option A: Delete the request entirely (Cleaner table)
            await sql.query`
                DELETE FROM Enrollments 
                WHERE EnrollmentID = ${enrollmentId}
            `;
            
            // Option B: If you prefer to keep a record of rejection, use this instead:
            // UPDATE Enrollments SET status = 'rejected' WHERE EnrollmentID = ${enrollmentId}
        }
...
const getMyCourses = async (req, res) => {
    try {
        ...
        // 🟢 THIS WAS MISSING: Loop through courses to attach students/requests
        for (let course of courses) {
...
const getCourseDetails = async (req, res) => {
    try {
        ...
        // 4. Get Assignments (Only one declaration of assignRes now)
        ...
        course.assignments = assignRes.recordset.map(a => {
            // Use the helper to find Title, Description, and ID no matter how SQL sends them
            ...
            return {
                id: realId, 
                courseId: courseId,
                title: realTitle,      // This uses the discovered title
        ...
        console.error("Backend Error:", error); // This prints the error to your terminal so you can see it
        res.status(400).json({ status: "fail", message: error.message });
    }
};
...
const addAssignment = async (req, res) => {
    try {
        ...
        // 🟢 UPDATED: Added 'OUTPUT INSERTED.*' to return the new data
        const result = await sql.query`
        ...
        // 🟢 UPDATED: Sending the actual assignment data back to the frontend
...
const submitAssignment = async (req, res) => {
    try {
        ...
        // (Using simple Insert - Make sure you have a 'Submissions' table!)
        await sql.query`
        ...
        // 3. SEND SUCCESS RESPONSE (This is likely where your old code crashed)
        res.status(200).json({ status: "success", message: "Assignment submitted successfully!" });

    } catch (error) {
        console.error("Submission Error:", error);
        // Even if it fails, send a JSON response so the frontend doesn't hang
        res.status(500).json({ status: "fail", message: error.message });
    }
};
...
const getSubmissionsForAssignment = async (req, res) => {
    try {
        const assignmentId = req.params.assId; // Assuming you pass Assignment ID in the route

        // Fetch submissions along with student details
        const result = await sql.query(`
            SELECT 
                S.SubmissionID, 
                S.FilePath, 
        ...
        // Format FilePath into a downloadable link for the teacher
        const submissions = result.recordset.map(s => ({
            ...
            // Ensure ID is lowercase for frontend
            id: s.SubmissionID
        }));
...
const gradeSubmission = async (req, res) => {
    try {
        ...
        // Update the Submission table with the grade and feedback
        await sql.query`
            UPDATE Submissions 
...
module.exports = { 
    ...
    getCourseDetails, addAnnouncement, addLecture, addAssignment, submitAssignment, getSubmissionsForAssignment, 
};
